---
title: "STAT 235 Midterm Project"
author: "Lee Kapp"
date: "10/16/2020"
output:
...
### Looking at power law distriubution of mutations
...
### Total number of mutations per sequence type by plasmicity
```{r featPlot, echo=FALSE, message=FALSE}
# homoplasmic
...
```{r perBasePair, echo=FALSE, message=FALSE, warning=FALSE}
...
### Are different types of mutations equally likely? Do they each affect the same sequence types equally?
```{r mutationTypes, echo=FALSE}
...
### Do homoplasmic and heteroplasmic mutations occur with equal frequency?
```{r HomvsHet genes, echo=FALSE}
...
### Looking at mutation types in highly affected genes
```{r mutationDist, echo=FALSE}
...
### Making sequence type-sepecific data frames
```{r individualGenes, echo=FALSE}
...
### tRNA variants by plasmicity
```{r tRNAGenes, echo=FALSE}
...
### rRNA variants by plasmicity
```{r rRNAGenes, echo=FALSE}
ggplot(data = rRNAs, mapping = aes(x = gene, y = counts_hom, fill = mutation)) +
  geom_col() +
  labs(title = "Most homoplasmic rRNA mutations are transitions", x = "rRNA gene", y = "Number of times mutation detected")  +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.title = element_text(size = 10))


ggplot(data = rRNAs, mapping = aes(x = gene, y = counts_het, fill = mutation)) +
  geom_col() +
  labs(title = "In-del rRNA mutations must be heteroplasmic", x = "rRNA gene", y = "Number of times mutation detected") +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.title = element_text(size = 10))
```


### Protein coding variants by plasmicity
```{r proteinCoding, echo=FALSE}
ggplot(data = proteins, mapping = aes(x = gene, y = counts_hom, fill = mutation)) +
  geom_col() +
  labs(title = "Transitions predominate homoplasmic mutations affecting proteins", x = "protein gene", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))

ggplot(data = proteins, mapping = aes(x = gene, y = counts_het, fill = mutation)) +
  geom_col() +
  labs(title = "Heteroplasmic mutations affecting proteins are more diverse", x = "protein gene", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))
```
...
### Non-coding variants by plasmicity
```{r nonCoding, echo=FALSE}
ggplot(data = non_coding, mapping = aes(x = gene, y = counts_hom, fill = mutation)) +
  geom_col() +
  labs(title = "The control region has the most homoplasmic mutations", x = "non-coding sequence", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))

ggplot(data = non_coding, mapping = aes(x = gene, y = counts_het, fill = mutation)) +
  geom_col() +
  labs(title = "The control region has the most heteroplasmic mutations", x = "non-coding sequence", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))

# filtering out CR regions in D loop that are highly variable
noCR<-non_coding %>% filter(gene != "MT-CR" && gene != "MT-CRb")

ggplot(data = noCR, mapping = aes(x = gene, y = counts_hom, fill = mutation)) +
  geom_col() +
  labs(title = "Non-coding homoplasmic mutations excluding CR & CRb", x = "non-coding sequence", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))

ggplot(data = noCR, mapping = aes(x = gene, y = counts_het, fill = mutation)) +
  geom_col() +
  labs(title = "Non-coding heteroplasmic mutations excluding CR & CRb", x = "non-coding sequence", y = "Number of times mutation detected") +
  theme(axis.title = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 12)) +
  theme(axis.text = element_text(angle = 90))
```

### Various tables of genes, sequence types, mutation rates by plasmicity
```{r tables}
...
### Making 2x2 tables and testing for equality of proportions
```{r 2x2Tables}
...
### Chi-squared tests with residuals
```{r total levels chisq}
...
```{r mutation by feature}
tfb<-chisq.test(tf_binary)
tfb
tfb$expected
tmb<-chisq.test(tm_binary)
tmb
tmb$expected
 
mutFeattable <- xtabs(rate~mutation + feature, data=mutRates)
mutFeattable
mF<-chisq.test(mutFeattable)
mF
mF$expected
mF$residuals
```

### Substitution rates per sequence type by plasmicity
```{r ind rates}
